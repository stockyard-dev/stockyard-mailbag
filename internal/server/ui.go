...
async function load(){var r=await fetch(A+'/emails').then(function(r){return r.json()});items=r.emails||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.subject||'').toLowerCase().includes(q)||(i.from_addr||'').toLowerCase().includes(q)||(i.to_addr||'').toLowerCase().includes(q)||(i.body||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No items found.</div>';return;}
var h='';f.forEach(function(i){
h+='<div class="item"><div class="item-top"><div style="flex:1"><div class="item-title">'+esc(i.subject)+'</div>';
if(i.from_addr)h+='<div class="item-sub">'+esc(i.from_addr)+'</div>';
...
if(i.to_addr)h+='<span>'+esc(i.to_addr)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.thread_id)h+='<span>'+esc(i.thread_id)+'</span>';
if(i.has_attachment)h+='<span>'+i.has_attachment+'</span>';
if(i.received_at)h+='<span>'+esc(i.received_at)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/emails/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{subject:"",from_addr:"",to_addr:"",body:"",status:"",thread_id:"",has_attachment:0,received_at:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' MAILBAG</h2>';
h+='<div class="fr"><label>Subject *</label><input id="f-subject" value="'+esc(i.subject)+'"></div>';
h+='<div class="fr"><label>From Addr</label><input id="f-from-addr" value="'+esc(i.from_addr)+'"></div>';
h+='<div class="fr"><label>To Addr</label><input id="f-to-addr" value="'+esc(i.to_addr)+'"></div>';
...
h+='<div class="fr"><label>Thread I D</label><input id="f-thread-id" value="'+esc(i.thread_id)+'"></div>';
h+='<div class="fr"><label>Has Attachment</label><input id="f-has-attachment" type="number" value="'+(i.has_attachment||0)+'"></div>';
h+='<div class="fr"><label>Received At</label><input id="f-received-at" type="date" value="'+esc(i.received_at)+'"></div>';
...
async function submit(){var body={subject:document.getElementById('f-subject').value.trim(),from_addr:document.getElementById('f-from-addr').value.trim(),to_addr:document.getElementById('f-to-addr').value.trim(),body:document.getElementById('f-body').value.trim(),status:document.getElementById('f-status').value.trim(),thread_id:document.getElementById('f-thread-id').value.trim(),has_attachment:parseInt(document.getElementById('f-has-attachment').value)||0,received_at:document.getElementById('f-received-at').value.trim()};
if(editId){await fetch(A+'/emails/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/emails',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
