...
func main(){port:=os.Getenv("PORT");if port==""{port="8750"};dataDir:=os.Getenv("DATA_DIR");if dataDir==""{dataDir="./mailbag-data"}
db,err:=store.Open(dataDir);if err!=nil{log.Fatalf("mailbag: %v",err)};defer db.Close();srv:=server.New(db)
