...
import (
	...
	"github.com/stockyard-dev/stockyard-mailbag/internal/license"
	"github.com/stockyard-dev/stockyard-mailbag/internal/server"
	"github.com/stockyard-dev/stockyard-mailbag/internal/store"
)

func main() {
	port := getEnv("PORT", "9230")
	dataDir := getEnv("DATA_DIR", "./data")
	licenseKey := os.Getenv("MAILBAG_LICENSE_KEY")
	...
	log.Printf("Stockyard Mailbag listening on :%s (tier: %s)", port, tier)
